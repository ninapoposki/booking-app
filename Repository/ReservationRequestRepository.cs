...
namespace BookingApp.Repository
{
    public  class ReservationRequestRepository : IReservationRequestRepository
    {
        ...
        public ReservationRequestRepository(){
            ...
            subject = new Subject();  }
        public List<ReservationRequest> GetAll(){ return serializer.FromCSV(FilePath); }
        ...
        public int NextId(){
            reservationRequests = serializer.FromCSV(FilePath);
            if (reservationRequests.Count < 1) { return 1; }
        ...
        public List<(DateTime, DateTime)> GenerateNewDateRange(DateTime startDate, int daysToStay){
            ...
            for (DateTime date = startDate; date <= endDate; date = date.AddDays(1)) {
                dates.Add((date, date.AddDays(daysToStay - 1)));
            }
            return dates;
        }
        ...
        public void DeleteById(int reservationId){
            reservationRequests = serializer.FromCSV(FilePath);
            List<ReservationRequest> requestsToDelete = reservationRequests.Where(r => r.ReservationId == reservationId).ToList();
            foreach (var requestToDelete in requestsToDelete){  reservationRequests.Remove(requestToDelete); }
            serializer.ToCSV(FilePath, reservationRequests);
            subject.NotifyObservers();
        }
        public void UpdateStatus(int accommodationReservationId, RequestStatus status, string Comment){
            ...
            if (reservationRequest != null){
                ...
                subject.NotifyObservers();  }
            else{  throw new Exception("Cannot find request"); }
        }
